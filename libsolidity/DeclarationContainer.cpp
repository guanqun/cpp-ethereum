...
bool DeclarationContainer::registerDeclaration(Declaration const& _declaration, bool _update)
{
	...
	if (!_update)
	{
		if (dynamic_cast<FunctionDefinition const*>(&_declaration))
		{
			...
			if (m_declarations.count(_declaration.getName()) != 0)
				return false;
...
bool DeclarationContainer::checkDuplicateFunctionDeclaration()
{
	for (auto&& pp: m_declarations)
		// if there are multiple declarations
		if (pp.second.size() > 1)
			for (auto&& declaration: pp.second)
				for (auto&& anotherDeclaration: pp.second)
				{
					// compare by address
					if (declaration < anotherDeclaration &&
						declaration->getScope() == anotherDeclaration->getScope() &&
						dynamic_cast<FunctionType const*>(declaration->getType().get())->getCanonicalSignature() ==
						dynamic_cast<FunctionType const*>(anotherDeclaration->getType().get())->getCanonicalSignature())
							return false;
				}
	return true;
}
